...
export async function renderEstudio(container) {
    ...
    photosHtml += `
      <div class="studio-photo-item" draggable="true" data-index="${idx}"
        ...
        <div class="studio-overlay" style="position:absolute; inset:0; background:rgba(0,0,0,0.5); opacity:0; transition:opacity 0.2s; display:flex; align-items:center; justify-content:center; gap:0.5rem;">
          <button onclick="event.stopPropagation(); deleteStudioPhoto(${idx})" style="background:#ef4444; color:white; padding:0.5rem; border-radius:9999px; border:none; cursor:pointer;">🗑️</button>
        </div>
  ...
  container.innerHTML = `
    <div style="display:flex; flex-direction:column; gap:1.5rem;">
      ...
      <div style="border:1px solid #374151; border-radius:0.75rem; background:#1f2937; padding:1.5rem; display:flex; flex-direction:column; gap:1rem;">
        <h3 style="font-size:1rem; font-weight:600; color:#d1d5db;">Apresentação</h3>
        <div>
          <label style="display:block; font-size:0.75rem; font-weight:500; color:#9ca3af; margin-bottom:0.25rem;">Título</label>
        ...
        <div>
          <label style="display:block; font-size:0.75rem; font-weight:500; color:#9ca3af; margin-bottom:0.25rem;">Descrição</label>
      ...
      <!-- Informações -->
      <div style="border:1px solid #374151; border-radius:0.75rem; background:#1f2937; padding:1.5rem; display:flex; flex-direction:column; gap:1rem;">
        <h3 style="font-size:1rem; font-weight:600; color:#d1d5db;">Informações</h3>
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem;">
          <div>
            <label style="display:block; font-size:0.75rem; font-weight:500; color:#9ca3af; margin-bottom:0.25rem;">Endereço</label>
        ...
        <div>
          <label style="display:block; font-size:0.75rem; font-weight:500; color:#9ca3af; margin-bottom:0.25rem;">Horário de Atendimento</label>
      ...
      <div style="border:1px solid #374151; border-radius:0.75rem; background:#1f2937; padding:1.5rem; display:flex; flex-direction:column; gap:1rem;">
        <div style="display:flex; justify-content:space-between; align-items:center;">
          <div>
            ...
            <p style="font-size:0.75rem; color:#9ca3af;">Mensagens que aparecem na bolha flutuante em sequência</p>
          </div>
      ...
      <div style="border:1px solid #374151; border-radius:0.75rem; background:#1f2937; padding:1.5rem; display:flex; flex-direction:column; gap:1rem;">
        <div style="display:flex; justify-content:space-between; align-items:center;">
          <div>
            <h3 style="font-size:1rem; font-weight:600; color:#d1d5db;">Fotos do Estúdio</h3>
            <p style="font-size:0.75rem; color:#9ca3af;">Fotos exibidas na seção do estúdio</p>
          </div>
        ...
        ${studio.photos.length === 0 ? '<p style="color:#9ca3af; text-align:center; padding:2rem;">Nenhuma foto. Use o botão acima para adicionar.</p>' : ''}
      </div>
  ...
  container.querySelector('#saveStudioBtn').onclick = async () => {
    // Coletar mensagens WhatsApp editadas
    const msgs = [];
