...
O app atual usa a porta `3000`. O novo app DEVE usar uma porta diferente.
...
**NUNCA** use as portas `3000`, `80`, `443` ou `27017`.
...
Nginx (porta 80/443)
    ...
    |                                     +--> MongoDB: cliquezoom
    |
    |-- novoapp.com.br ------------> localhost:3001 (NOVO APP)
    |                                     |
    |                                     +--> MongoDB: novo_banco
    |
    |-- outroapp.com.br -----------> localhost:3002 (OUTRO APP)
    |                                     |
    |                                     +--> MongoDB: outro_banco
    |
    v
...
1. **NUNCA** altere a config Nginx do `cliquezoom` ao configurar um novo app
2. **NUNCA** use a porta `3000` (ja em uso pelo CLIQUE·ZOOM)
3. **NUNCA** use o banco `cliquezoom` no MongoDB (pertence ao CLIQUE·ZOOM)
